...
from src.icflow.engines.lvs_repair import LVSRepairMasterEngine
...
class TestLVSRepairMasterEngine:
    """LVS修复主引擎测试类"""
    ...
    def engine(self, message_bus):
        """创建引擎fixture"""
        # 创建引擎
        engine = LVSRepairMasterEngine({"default_tool": "calibre"})
        # 设置消息总线
    ...
    async def test_engine_initialization(self, engine):
        """测试引擎初始化"""
        assert engine.engine_id == "lvs_repair_master_engine"
        ...
        # 检查是否订阅了正确的事件类型
        assert DesignFlowEventTypes.LVS_VIOLATION_DETECTED in engine.subscribed_event_types
    ...
    @pytest.mark.asyncio
    @patch.object(LVSRepairMasterEngine, '_verify_repair_result', new_callable=AsyncMock, return_value=True)
    async def test_handle_lvs_violation(self, mock_verify, engine, message_bus):
        """测试处理LVS违例事件"""
        # 启动消息总线和引擎
        ...
        # 创建LVS违例事件
        lvs_event = FlowEvent(
            event_type=DesignFlowEventTypes.LVS_VIOLATION_DETECTED,
            source="test_source",
            payload={
                "task_id": "task_001",
        ...
        # 处理事件
        await engine.process(lvs_event)
    ...
    async def test_select_repair_strategy(self, engine):
        """测试修复策略选择"""
        # 已知策略
        assert engine._select_repair_strategy("short") == "auto_cut"
        assert engine._select_repair_strategy("open") == "auto_connect"
        assert engine._select_repair_strategy("mismatch") == "auto_adjust"
        assert engine._select_repair_strategy("floating") == "auto_ground"
    ...
    async def test_get_stats(self, engine):
        ...
        assert stats["engine_id"] == "lvs_repair_master_engine"
